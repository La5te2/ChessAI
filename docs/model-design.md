...
search backend 按 checkpoint `arch.type` 分派。`resnet_pv_linear_mcts` 使用 `alphazero_64x73` Policy/Value 与 FPU。`resnet_pva_gad_mcts` 使用 `sd_64x64_underpromo9` Policy/Value/Advantage，以 `V+A` 构造边 Q 先验，并在已访问节点上让该先验随 visits 衰减。
...
这样它不是把国际象棋变成非完全信息游戏。棋盘仍然是完全信息的。“战争迷雾”来自模型自身看不见的计算空间。有限容量、有限训练数据、有限搜索预算让许多后继局面对模型近似不可见。因此 `U_e` 可以被看作模型对自己盲区的估计。
