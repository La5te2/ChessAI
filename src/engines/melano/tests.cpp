	...
	try {
		...
		for (std::int64_t source = 0; source < relation_ids.size(0); ++source) {
			for (std::int64_t target = 0; target < relation_ids.size(1); ++target) {
				...
				require(relation >= 0 && relation < melano::kGeometryRelations, "geometry relation id is out of range");
		...
		require(active_relations == melano::kGeometryRelations, "geometry relation layout contains an unreachable id");
		...
		require(melano::move_to_index(king_side_castle) == 4 * 64 + 6, "king-side castling policy index mismatch");
		require(melano::move_to_index(queen_side_castle) == 4 * 64 + 2, "queen-side castling policy index mismatch");
		...
		require(melano::move_to_index(black_king_side) == 60 * 64 + 62, "black king-side castling policy index mismatch");
		require(melano::move_to_index(black_queen_side) == 60 * 64 + 58, "black queen-side castling policy index mismatch");
		...
		melano::preprocess_pgn(preprocess);
		{
			...
			require(supervised.info().length == 6, "Melano preprocessing rejected detached comments or retained invalid SAN");
			const auto supervised_batch = supervised.read_contiguous(0, 6);
			const auto expected_initial = melano::encode_boards({board}).index({0});
			require(torch::equal(supervised_batch.states.index({0}), expected_initial), "HDF5 initial state differs from live state codec");
			auto after_d4 = board;
			...
			require(torch::equal(supervised_batch.states.index({1}), expected_after_d4), "HDF5 second state differs from live state codec");
			require(supervised_batch.moves.index({0}).item<std::int64_t>() == melano::move_to_index(d4), "HDF5 policy target differs from live move codec");
			require(std::abs(supervised_batch.values.index({0}).item<float>()) < 1e-6F && std::abs(supervised_batch.values.index({1}).item<float>()) < 1e-6F &&
			        std::abs(supervised_batch.values.index({2}).item<float>()) < 1e-6F,
			    "Melano preprocessing changed neutral opening targets");
			require(supervised_batch.values.index({4}).item<float>() > 0.0F && supervised_batch.values.index({5}).item<float>() < 0.0F,
		...
		require(melano::index_to_move(melano::move_to_index(policy_result.move), board) == policy_result.move, "direct Policy inference selected an illegal move");
