...
int main(int argc, char **argv) {
	try {
		...
		if (args.has("help")) {
			std::cout << "Usage: search --model <melano.pth> [--fen <fen>] [options]\n"
		...
		options.repetition_policy_penalty = args.get_double("repetition-policy-penalty", options.repetition_policy_penalty);
		options.instant_mate_first = args.get_bool("instant-mate-first", options.instant_mate_first);
		...
		const std::string fen = fen_argument == "startpos" ? std::string(chess::constants::STARTPOS) : fen_argument;
		chess::Board board(fen);
		const auto result = searcher.search(board);

		std::cout << std::fixed << std::setprecision(6);
		std::cout << "fen: " << board.getFen() << '\n';
		std::cout << "best: " << melano::move_san(board, result.move) << ' ' << melano::move_uci(result.move) << '\n';
		std::cout << "value: " << result.value << '\n';
		std::cout << "mcts: " << result.sims_completed << " / " << result.dynamic_target << " / " << options.mcts_sims << '\n';
		...
		for (std::size_t index = 0; index < result.root.size(); ++index) {
			...
			std::cout << index + 1 << ". " << melano::move_san(board, row.move) << ' ' << melano::move_uci(row.move) << " p=" << row.probability
			          << " decision=" << row.decision_score << " prior=" << row.prior << " visits=" << row.visits << " q=" << row.q;
			if (row.instant_mate) {
				std::cout << " imf";
			}
