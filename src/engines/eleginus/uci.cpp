...
private:
	...
	void load_model() {
		if (model_ && loaded_path_ == model_path_) {
			return;
		}
	...
	int allocated_time(const std::unordered_map<std::string, std::string> &values) const {
		if (values.contains("infinite")) {
			return 0;
		}
		if (const auto found = values.find("movetime"); found != values.end()) {
			return std::max(1, parse_int(found->second, 1) - move_overhead_);
		}
	...
	try {
		...
		for (int index = 1; index < argc; ++index) {
			if (std::string(argv[index]) == "--model" && index + 1 < argc) {
				...
				throw std::invalid_argument("usage: uci [--model eleginus.pth]");
			}
		}
