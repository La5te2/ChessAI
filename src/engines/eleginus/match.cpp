...
namespace eleginus {
	...
	namespace {
		...
		double elo(double score) {
			if (score <= 0) return -std::numeric_limits<double>::infinity();
			if (score >= 1) return std::numeric_limits<double>::infinity();
	...
	std::vector<chess::Board> openings(const std::filesystem::path &path) {
		...
		if (!in) throw std::runtime_error("cannot open opening book: " + path.string());
		...
		if (in.bad() || in.gcount() != 0) throw std::runtime_error("invalid Polyglot opening book");
		if (!book.contains(chess::Board().hash())) throw std::runtime_error("opening book has no start-position moves");
	...
	int match(
		...
		for (;;) {
			...
			if (cancel && cancel()) return -1;
			if (resultMove.move.move() == chess::Move::NO_MOVE) throw std::runtime_error("match search returned no move for a nonterminal position");
	...
	MatchScore confidence(const std::array<int, 5> &pairs) {
		...
		double total = 0;
		for (int i = 0; i < 5; ++i) {
			if (pairs[i] < 0) throw std::invalid_argument("negative opening-pair count");
			count += pairs[i];
			total += 0.25 * i * pairs[i];
		}
		if (count != kOpeningPairs) throw std::invalid_argument("confidence requires 1000 completed opening pairs");
