...
namespace eleginus {
	...
	namespace {
		...
		public:
			...
			Value RIGHT(Value role, int side) const { return NUM((in[atomIndex(Atom::CR)] & (1ULL << (2 * color(role) + side))) != 0); }
		...
		private:
			...
			IS ringAttacks(IS attacker, IS defender, int type, int distance) {
				return distance == 1 ? CNT(b.AND(b.attacks(attacker, type), b.attacks(defender, 5))) : CNT(b.AND(b.attacks(attacker, type), kingRegion(defender, false)));
			}
			...
			IS escapes(IS defender, IS attacker) { return CNT(b.AND(b.attacks(defender, 5), b.AND(b.NOT(own(defender)), b.NOT(b.attacks(attacker))))); }
			...
			IS flank(IS map, IS defender) { return CNT(b.AND(map, kingRegion(defender, true))); }
