...
private:
	...
	int score_cp(float value) const { return static_cast<int>(std::lround(kCentipawnScale * std::tan(kCentipawnAngle * std::clamp(value, -1.0F, 1.0F)))); }
	...
	void emit_info(const gadus::SearchResult &result) const {
		...
		for (int index = 0; index < count; ++index) {
			...
			print("info depth " + std::to_string(depth) + " seldepth " + std::to_string(depth) + " multipv " + std::to_string(index + 1) + " score cp " +
			    std::to_string(score_cp(value)) + " nodes " + std::to_string(nodes) + " nps " + std::to_string(nps) + " time " + std::to_string(elapsed) + " pv " +
	...
	void go(const std::string &line) {
		...
		search_thread_ = std::thread([this, board, searcher, progress_interval_ms] {
			try {
				const auto result =
				    searcher->search(board, [this](const gadus::SearchResult &partial) { emit_info(partial); }, progress_interval_ms, [this] { return stop_requested_.load(); });
				emit_info(result);
				...
				print("bestmove " + (moves.empty() ? std::string("0000") : gadus::move_uci(moves.front())));
			}
		});
	}
