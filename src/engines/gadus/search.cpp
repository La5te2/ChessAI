...
int main(int argc, char **argv) {
	try {
		...
		if (args.has("help")) {
			std::cout << "Usage: search --model <gadus.pth> [--fen <fen>] [options]\n"
		...
		std::cout << "best: " << gadus::move_san(board, result.move) << ' ' << gadus::move_uci(result.move) << '\n';
		...
		for (std::size_t index = 0; index < result.root.size(); ++index) {
			...
			std::cout << index + 1 << ". " << gadus::move_san(board, row.move) << ' ' << gadus::move_uci(row.move) << " p=" << row.probability << " decision=" << row.decision_score
			          << " prior=" << row.prior << " visits=" << row.visits << " q=" << row.q;
			if (row.instant_mate) {
				std::cout << " imf";
			}
