...
int main() {
	try {
		...
		require(aligned > 0.5 && aligned < 1.0, "aligned gradients did not smoothly increase the Value weight");

		gadus::ValueWeightController conflicting_weight(0.5);
		const double conflicting = conflicting_weight.update(4.0, 1.0, -1.5);
		require(conflicting > 0.5 && conflicting < 1.5, "conflicting gradients did not move smoothly toward the feasible interval");
		...
		require(minimum_weight.value() == 0.2 && maximum_weight.value() == 2.0, "Value-weight boundary was not preserved");
		...
		require(gadus::terminal_value_side_to_move(checkmate) == -1.0F, "checkmate side-to-move value mismatch");
		require(gadus::game_result(checkmate) == "1-0", "checkmate result mismatch");
		require(gadus::game_termination(checkmate) == "checkmate", "checkmate termination mismatch");

		chess::Board stalemate("7k/5Q2/6K1/8/8/8/8/8 b - - 0 1");
		require(gadus::game_is_over(stalemate), "stalemate was not detected");
		require(gadus::terminal_value_side_to_move(stalemate) == 0.0F, "stalemate side-to-move value mismatch");
		require(gadus::game_termination(stalemate) == "stalemate", "stalemate termination mismatch");

		chess::Board insufficient("7k/8/8/8/8/8/8/K7 w - - 0 1");
		require(gadus::game_termination(insufficient) == "insufficient material", "insufficient-material termination mismatch");
		chess::Board bishop_only("7k/8/8/8/8/8/8/KB6 w - - 0 1");
		require(gadus::game_termination(bishop_only) == "insufficient material", "king-and-bishop termination mismatch");
		chess::Board knight_only("7k/8/8/8/8/8/8/KN6 w - - 0 1");
		require(gadus::game_termination(knight_only) == "insufficient material", "king-and-knight termination mismatch");
		chess::Board bishop_vs_knight("6nk/8/8/8/8/8/8/KB6 w - - 0 1");
		require(!gadus::game_is_over(bishop_vs_knight), "king-and-bishop versus king-and-knight was incorrectly adjudicated");

		chess::Board fifty_move("7k/8/8/8/8/8/6R1/K7 b - - 100 1");
		require(gadus::game_termination(fifty_move) == "fifty move rule", "fifty-move termination mismatch");
		...
		relation_block->project_relation_residual();
		const auto projected = relation_block->named_parameters()["relation_residual"].flatten(1);
		const auto expanded_index = displacement_index.flatten().reshape({1, -1}).expand({8, -1});
		auto displacement_sums = torch::zeros({8, 225}, projected.options());
		...
		require(policy.sizes() == torch::IntArrayRef({2, gadus::kActionSize}), "policy shape mismatch");
		...
		require_finite_gradients(model);
		{
			...
			for (const auto &parameter : model->named_parameters()) {
				if (parameter.key().ends_with("path_balance")) {
					parameter.value().copy_(torch::linspace(-1.5, 1.5, parameter.value().numel(), parameter.value().options()));
				}
			}
		}
		...
		require(torch::allclose(legal_output.second, reference.second), "legal-action forward changed Value");
		...
		require(torch::allclose(black_legal.first, black_reference.first.gather(1, black_indices), 1e-5, 1e-6), "black legal-action projection used noncanonical indices");
		gadus::save_checkpoint_atomic(checkpoint, model, {8, 1});
		{
			torch::serialize::InputArchive root_archive;
			root_archive.load_from(checkpoint.string(), torch::Device(torch::kCPU));
			...
			backbone_archive.read("4", block_archive);
			require(!block_archive.try_read("displacement_index", omitted, true), "checkpoint retained reconstructible displacement geometry");
		}
		...
		require(torch::allclose(reference.first, loaded_output.first), "checkpoint changed policy output");
		require(torch::allclose(reference.second, loaded_output.second), "checkpoint changed value output");
		loaded->fuse_for_inference();
		auto fused_output = loaded->forward(gadus::encode_boards({board}));
		auto fused_legal_output = loaded->forward_legal(gadus::encode_boards({board}), legal_indices);
		require(torch::allclose(loaded_output.first, fused_output.first, 1e-4, 1e-5), "Conv-BN fusion changed policy output");
		require(torch::allclose(loaded_output.second, fused_output.second, 1e-5, 1e-6), "Conv-BN fusion changed value output");
		require(torch::allclose(fused_legal_output.first, fused_output.first.gather(1, legal_indices), 1e-4, 1e-5), "fused legal-action forward changed a requested policy logit");
		require(torch::allclose(fused_legal_output.second, fused_output.second, 1e-5, 1e-6), "fused legal-action forward changed Value");
		...
		require(compact_result[0].legal_indices.size() == compact_result[0].legal_policy.size(), "compact Gadus evaluation width mismatch");
		for (std::size_t column = 0; column < compact_result[0].legal_indices.size(); ++column) {
			require(std::abs(compact_result[0].legal_policy[column] - direct_result.policy[compact_result[0].legal_indices[column]]) < 1e-6F,
		...
		for (int action = 0; action < gadus::kActionSize; ++action) {
			require(std::abs(direct_result.policy[action] - expected_policy[action]) < 1e-5F, "compact legal-policy transfer changed direct search probabilities");
		}
		...
		require(first_cached.nn_evaluations == 1 && first_cached.evaluation_reuses == 0, "first persistent-cache search did not evaluate its root");
		require(second_cached.nn_evaluations == 0 && second_cached.evaluation_reuses == 1, "second persistent-cache search did not reuse its root evaluation");
		require(second_cached.sims_completed == 0 && second_cached.expanded_nodes == 1, "persistent evaluation cache retained search-tree statistics");
		require(torch::allclose(torch::tensor(first_cached.policy), torch::tensor(second_cached.policy), 1e-6, 1e-7), "persistent evaluation cache changed Policy");
		require(std::abs(first_cached.value - second_cached.value) < 1e-7F, "persistent evaluation cache changed Value");
		cached_searcher.clear_evaluation_cache();
		const auto after_clear = cached_searcher.search(board);
		require(after_clear.nn_evaluations == 1 && after_clear.evaluation_reuses == 0, "clearing the persistent evaluation cache did not force reevaluation");
		...
		require(coverage_result.root.size() == start_moves.size(), "root coverage omitted a legal action from diagnostics");
		for (int index = 0; index < active_root_width; ++index)
			require(coverage_result.root[static_cast<std::size_t>(index)].visits == 1, "legal root action did not receive its fair visit");

		auto competition_options = coverage_options;
		competition_options.mcts_sims += 1;
		gadus::Searcher competition_searcher(loaded, torch::Device(torch::kCPU), competition_options);
		const auto competition_result = competition_searcher.search(board);
		const auto represented = std::count_if(competition_result.policy.begin(), competition_result.policy.end(), [](float score) { return score > 0.0F; });
		require(competition_result.sims_completed == competition_options.mcts_sims, "root competition changed the simulation budget");
		const int competition_visits =
		    std::accumulate(competition_result.root.begin(), competition_result.root.end(), 0, [](int total, const gadus::RootMove &move) { return total + move.visits; });
		require(competition_visits == competition_result.sims_completed, "root competition visits do not match completed simulations");
		require(represented == active_root_width, "root competition omitted a legal action");
		for (const auto &root_move : competition_result.root)
			require(std::abs(root_move.decision_score - root_move.probability) < 1e-6F, "root decision score differs from the PUCT visit distribution");
		...
		while (static_cast<int>(std::floor(10.0 * std::log1p(static_cast<double>(fair_budget) / (10.0 * static_cast<double>(legal_width))))) < 2) {
			++fair_budget;
		}
		...
		for (int index = 0; index < active_root_width; ++index)
			require(fair_result.root[static_cast<std::size_t>(index)].visits >= 2, "legal root action did not satisfy its fair visit floor");
		...
		require(narrow_result.move == competition_result.move, "MultiPV width changed the selected move");
		require(torch::allclose(torch::tensor(narrow_result.policy), torch::tensor(competition_result.policy), 1e-6, 1e-7), "MultiPV width changed root allocation");

		// A played searched move reuses its evaluation even when it is not bestmove.
		...
		require(trajectory_root.root.size() > 1, "trajectory-cache test requires a non-best searched move");
		...
		require(trajectory_next.evaluation_reuses > 0, "trajectory-aware cache did not reuse the searched child root");
		require(trajectory_next.sims_completed == trajectory_options.mcts_sims, "trajectory-aware cache changed the simulation budget");
