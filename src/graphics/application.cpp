...
constexpr ImU32 color32(float red, float green, float blue, float alpha = 1.0F) {
	return IM_COL32(static_cast<int>(red * 255.0F), static_cast<int>(green * 255.0F), static_cast<int>(blue * 255.0F), static_cast<int>(alpha * 255.0F));
}
...
std::filesystem::path executable_settings_path(const char *argument_zero) {
	...
	if (argument_zero != nullptr && argument_zero[0] != '\0') {
		auto executable = std::filesystem::absolute(std::filesystem::path(argument_zero), error);
		if (!error) {
...
BoardLayout board_layout(float available_width, float available_height, float board_fraction) {
	constexpr float splitter_width = 8.0F;
	const float right_minimum = std::min(360.0F, available_width * 0.42F);
	const float bottom_controls = ImGui::GetFrameHeight() + ImGui::GetStyle().ItemSpacing.y * 2.0F + ImGui::GetStyle().WindowPadding.y * 2.0F + 4.0F;
	const float maximum_board_size = std::max(1.0F, std::min(available_height - bottom_controls, available_width - right_minimum - splitter_width));
	const float minimum_board_size = std::min(280.0F, maximum_board_size);
	const float board_size = std::clamp(available_width * board_fraction, minimum_board_size, maximum_board_size);
...
bool toggle_switch(const char *label, bool &value) {
	...
	const auto background = ImGui::GetColorU32(value ? ImGuiCol_SliderGrabActive : ImGuiCol_FrameBg);
	const auto knob = ImGui::GetColorU32(ImGuiCol_Text);
	auto *draw = ImGui::GetWindowDrawList();
	draw->AddRectFilled(position, {position.x + width, position.y + height}, background, height * 0.5F);
	const float radius = height * 0.38F;
	const float center_x = value ? position.x + width - height * 0.5F : position.x + height * 0.5F;
	draw->AddCircleFilled({center_x, position.y + height * 0.5F}, radius, knob, 20);
	ImGui::PopID();
	return pressed;
}
...
bool centered_step_button(const char *identifier, bool increment) {
	...
	auto *draw = ImGui::GetWindowDrawList();
	draw->AddLine({center.x - half_extent, center.y}, {center.x + half_extent, center.y}, color, thickness);
	if (increment) {
		draw->AddLine({center.x, center.y - half_extent}, {center.x, center.y + half_extent}, color, thickness);
	}
	return pressed;
}
...
bool step_input(const char *label, int &value) {
	...
	const float input_width = std::max(1.0F, ImGui::CalcItemWidth() - 2.0F * (button_extent + style.ItemInnerSpacing.x));
	ImGui::SetNextItemWidth(input_width);
	bool changed = ImGui::InputScalar("##value", ImGuiDataType_S32, &value, nullptr, nullptr);
	ImGui::SameLine(0.0F, style.ItemInnerSpacing.x);
	if (centered_step_button("##decrement", false) && value > std::numeric_limits<int>::min()) {
		--value;
		changed = true;
	}
	ImGui::SameLine(0.0F, style.ItemInnerSpacing.x);
	if (centered_step_button("##increment", true) && value < std::numeric_limits<int>::max()) {
		++value;
		changed = true;
	}
	ImGui::SameLine(0.0F, style.ItemInnerSpacing.x);
...
bool step_input(const char *label, std::uint64_t &value) {
	...
	const float input_width = std::max(1.0F, ImGui::CalcItemWidth() - 2.0F * (button_extent + style.ItemInnerSpacing.x));
	ImGui::SetNextItemWidth(input_width);
	bool changed = ImGui::InputScalar("##value", ImGuiDataType_U64, &value, nullptr, nullptr);
	ImGui::SameLine(0.0F, style.ItemInnerSpacing.x);
	...
	ImGui::SameLine(0.0F, style.ItemInnerSpacing.x);
	if (centered_step_button("##increment", true) && value < std::numeric_limits<std::uint64_t>::max()) {
		++value;
		changed = true;
	}
	ImGui::SameLine(0.0F, style.ItemInnerSpacing.x);
...
bool copy_button(const char *identifier) {
	...
	if (hovered) {
		...
		const ImVec4 neutral = {
		    background.x * (1.0F - foreground_mix) + foreground.x * foreground_mix,
		    background.y * (1.0F - foreground_mix) + foreground.y * foreground_mix,
		    background.z * (1.0F - foreground_mix) + foreground.z * foreground_mix,
		    1.0F,
		};
		ImGui::GetWindowDrawList()->AddRectFilled(minimum, maximum, ImGui::GetColorU32(neutral), 2.0F);
	}
	const auto color = ImGui::GetColorU32(hovered ? ImGuiCol_Text : ImGuiCol_TextDisabled);
	const float unit = side / 16.0F;
	auto *draw = ImGui::GetWindowDrawList();
	draw->AddRect({minimum.x + 6.0F * unit, minimum.y + 2.0F * unit}, {minimum.x + 14.0F * unit, minimum.y + 10.0F * unit}, color, 1.0F, 0, std::max(1.0F, unit));
	draw->AddRect({minimum.x + 2.0F * unit, minimum.y + 6.0F * unit}, {minimum.x + 10.0F * unit, minimum.y + 14.0F * unit}, color, 1.0F, 0, std::max(1.0F, unit));
	ImGui::PopID();
	return pressed;
}
...
void read_engine_json(const nlohmann::json &json, EngineConfig &config) {
	...
	if (json.contains("option_definitions") && json["option_definitions"].is_array()) {
		for (const auto &entry : json["option_definitions"]) {
			...
			option.choices = entry.value("choices", std::vector<std::string>{});
...
const nlohmann::json *find_option_value(const nlohmann::json &options, const std::string &name) {
...
void write_option_value(nlohmann::json &options, const std::string &name, const nlohmann::json &value) {
	const auto target = normalized_option_name(name);
...
void set_uci_option(EngineConfig &config, const std::string &name, const nlohmann::json &value) {
...
std::string score_text(const AnalysisLine &line) {
	...
	score << std::showpos << std::fixed << std::setprecision(2) << static_cast<double>(line.score) / 100.0;
	return score.str();
}

/// Converts a UCI principal variation to SAN from a supplied root position.
std::string pv_san(const std::string &fen, const std::vector<std::string> &pv, std::size_t maximum = 10) {
	try {
...
std::optional<std::filesystem::path> file_dialog(bool save, const wchar_t *filter, const wchar_t *extension) {
#ifdef _WIN32
...
void polygon(ImDrawList *draw, const std::vector<ImVec2> &points, ImU32 fill, ImU32 outline, float thickness) {
	draw->AddConvexPolyFilled(points.data(), static_cast<int>(points.size()), fill);
	draw->AddPolyline(points.data(), static_cast<int>(points.size()), outline, ImDrawFlags_Closed, thickness);
}

/// Draws one built-in procedural chess piece at any board scale.
void draw_vector_piece(ImDrawList *draw, const chess::Piece &piece, ImVec2 center, float size) {
	const bool white = piece.color() == chess::Color::WHITE;
	const ImU32 fill = white ? color32(0.96F, 0.97F, 0.96F) : color32(0.12F, 0.15F, 0.16F);
	const ImU32 outline = white ? color32(0.15F, 0.19F, 0.19F) : color32(0.90F, 0.92F, 0.90F);
	...
	case chess::PieceType::underlying::PAWN:
		...
		polygon(draw, {{x - size * 0.10F, y - size * 0.09F}, {x + size * 0.10F, y - size * 0.09F}, {x + size * 0.18F, y + size * 0.23F}, {x - size * 0.18F, y + size * 0.23F}},
		    fill, outline, stroke);
		base(size * 0.52F, size * 0.20F, size * 0.30F);
		break;
	case chess::PieceType::underlying::ROOK:
		polygon(draw,
		    {{x - size * 0.24F, y - size * 0.28F}, {x - size * 0.24F, y - size * 0.13F}, {x - size * 0.17F, y - size * 0.13F}, {x - size * 0.17F, y + size * 0.21F},
		        {x + size * 0.17F, y + size * 0.21F}, {x + size * 0.17F, y - size * 0.13F}, {x + size * 0.24F, y - size * 0.13F}, {x + size * 0.24F, y - size * 0.28F},
		        {x + size * 0.10F, y - size * 0.28F}, {x + size * 0.10F, y - size * 0.20F}, {x - size * 0.02F, y - size * 0.20F}, {x - size * 0.02F, y - size * 0.28F}},
		    fill, outline, stroke);
		base(size * 0.62F, size * 0.19F, size * 0.30F);
		break;
	case chess::PieceType::underlying::KNIGHT:
		polygon(draw,
		    {{x - size * 0.23F, y + size * 0.22F}, {x - size * 0.17F, y - size * 0.06F}, {x - size * 0.02F, y - size * 0.27F}, {x + size * 0.17F, y - size * 0.16F},
		        {x + size * 0.23F, y + size * 0.08F}, {x + size * 0.11F, y + size * 0.22F}},
		    fill, outline, stroke);
		draw->AddCircleFilled({x + size * 0.075F, y - size * 0.13F}, size * 0.025F, outline, 12);
		base(size * 0.63F, size * 0.20F, size * 0.30F);
		break;
	case chess::PieceType::underlying::BISHOP:
		polygon(draw,
		    {{x, y - size * 0.31F}, {x + size * 0.14F, y - size * 0.11F}, {x + size * 0.08F, y + size * 0.03F}, {x + size * 0.16F, y + size * 0.22F},
		        {x - size * 0.16F, y + size * 0.22F}, {x - size * 0.08F, y + size * 0.03F}, {x - size * 0.14F, y - size * 0.11F}},
		    fill, outline, stroke);
		draw->AddLine({x - size * 0.02F, y - size * 0.24F}, {x + size * 0.07F, y - size * 0.10F}, outline, stroke);
		base(size * 0.61F, size * 0.20F, size * 0.30F);
		break;
	case chess::PieceType::underlying::QUEEN:
		polygon(draw,
		    {{x - size * 0.25F, y - size * 0.18F}, {x - size * 0.13F, y - size * 0.29F}, {x, y - size * 0.16F}, {x + size * 0.13F, y - size * 0.29F},
		        {x + size * 0.25F, y - size * 0.18F}, {x + size * 0.15F, y + size * 0.21F}, {x - size * 0.15F, y + size * 0.21F}},
		    fill, outline, stroke);
		for (const float offset : {-0.13F, 0.0F, 0.13F}) {
			draw->AddCircleFilled({x + offset * size, y - size * 0.29F}, size * 0.035F, fill, 16);
			draw->AddCircle({x + offset * size, y - size * 0.29F}, size * 0.035F, outline, 16, stroke);
		}
		base(size * 0.66F, size * 0.19F, size * 0.30F);
		break;
	case chess::PieceType::underlying::KING:
		draw->AddLine({x, y - size * 0.36F}, {x, y - size * 0.17F}, outline, stroke * 1.3F);
		draw->AddLine({x - size * 0.08F, y - size * 0.28F}, {x + size * 0.08F, y - size * 0.28F}, outline, stroke * 1.3F);
		polygon(draw,
		    {{x, y - size * 0.19F}, {x + size * 0.16F, y - size * 0.04F}, {x + size * 0.11F, y + size * 0.21F}, {x - size * 0.11F, y + size * 0.21F},
...
void draw_piece(ImDrawList *draw, std::string_view style, const chess::Piece &piece, ImVec2 center, float size) {
	if (style != "vector" && draw_compiled_piece(draw, style, piece, center, size * 0.94F)) {
		return;
	}
...
std::optional<int> draw_board(
    const GameState &game, float size, bool flipped, const Appearance &appearance, std::optional<int> selected, const std::vector<chess::Move> &targets, const char *identifier) {
	ImGui::PushID(identifier);
	...
	for (int screen_rank = 0; screen_rank < 8; ++screen_rank) {
		for (int screen_file = 0; screen_file < 8; ++screen_file) {
			...
			ImU32 color = packed_color((file + rank) % 2 == 0 ? appearance.dark : appearance.light);
			...
			const bool legal_target = std::any_of(targets.begin(), targets.end(), [&](const auto &move) { return move.to().index() == square; });
			if (legal_target) {
				const bool occupied = game.board().at(chess::Square(square)) != chess::Piece::NONE;
				if (occupied) {
					draw->AddCircle({minimum.x + square_size * 0.5F, minimum.y + square_size * 0.5F}, square_size * 0.43F, packed_color(appearance.legal), 32,
					    std::max(3.0F, square_size * 0.07F));
				} else {
					draw->AddCircleFilled({minimum.x + square_size * 0.5F, minimum.y + square_size * 0.5F}, square_size * 0.12F, packed_color(appearance.legal), 24);
				}
			}

			const auto piece = game.board().at(chess::Square(square));
			if (piece != chess::Piece::NONE) {
				draw_piece(draw, appearance.piece_style, piece, {minimum.x + square_size * 0.5F, minimum.y + square_size * 0.49F}, square_size);
			}

			if (appearance.coordinates) {
				const bool label_rank = screen_file == 0;
				const bool label_file = screen_rank == 7;
				const ImU32 label_color = packed_color((file + rank) % 2 == 0 ? appearance.light : appearance.dark);
				if (label_rank) {
					const std::string label(1, static_cast<char>('1' + rank));
					draw->AddText({minimum.x + 4.0F, minimum.y + 2.0F}, label_color, label.c_str());
				}
				if (label_file) {
					const std::string label(1, static_cast<char>('a' + file));
					const auto dimensions = ImGui::CalcTextSize(label.c_str());
					draw->AddText({maximum.x - dimensions.x - 4.0F, maximum.y - dimensions.y - 2.0F}, label_color, label.c_str());
				}
			}
		}
	}
	draw->AddRect(origin, {origin.x + size, origin.y + size}, color32(0.08F, 0.10F, 0.11F), 0.0F, 0, std::max(1.0F, size * 0.003F));

	std::optional<int> result;
	if (clicked && mouse.x >= origin.x && mouse.x < origin.x + size && mouse.y >= origin.y && mouse.y < origin.y + size) {
...
void analysis_table(const AnalysisSnapshot &snapshot, const std::string &fen, float height) {
	if (ImGui::BeginChild("analysis-lines", {0.0F, height}, ImGuiChildFlags_Borders)) {
		if (!snapshot.error.empty()) {
			ImGui::TextColored({0.95F, 0.32F, 0.30F, 1.0F}, "%s", snapshot.error.c_str());
		} else if (snapshot.lines.empty()) {
			ImGui::TextDisabled(snapshot.searching ? "Waiting for analysis..." : "Analysis is idle");
		} else if (ImGui::BeginTable("multipv", 5,
		               ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable | ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_SizingStretchProp | ImGuiTableFlags_ScrollY)) {
			...
			ImGui::TableSetupColumn("#", ImGuiTableColumnFlags_WidthFixed, column_width("##", 32.0F));
			ImGui::TableSetupColumn("Move", ImGuiTableColumnFlags_WidthFixed, column_width("O-O-O+", 80.0F));
			ImGui::TableSetupColumn("Score", ImGuiTableColumnFlags_WidthFixed, column_width("-M123", 72.0F));
			ImGui::TableSetupColumn("Depth", ImGuiTableColumnFlags_WidthFixed, column_width("Depth", 64.0F));
...
public:
	...
	Application(int argc, char **argv) : settings_path_(executable_settings_path(argc > 0 ? argv[0] : nullptr)) {
	...
	~Application() { stadiums_.stop_all(); }
	...
	void handle_menu_command(MenuCommand command) {
		...
		case MenuCommand::SavePgn:
			if (mode_ == Mode::Simulator) {
				...
				save_pgn(stadium().game(), stadium().white_name(), stadium().black_name(), stadium().result(), stadium().termination());
			}
			break;
	...
	void frame(bool update_state = true) {
		...
		auto &io = ImGui::GetIO();
		const float window_ratio = std::max(0.25F, std::min(io.DisplaySize.x / 1280.0F, io.DisplaySize.y / 820.0F));
		const float viewport_scale = appearance_.auto_font_scale ? std::clamp(std::sqrt(window_ratio), 0.95F, 1.10F) : 1.0F;
		io.FontGlobalScale = std::clamp(appearance_.font_size * viewport_scale / 32.0F, 0.44F, 1.25F);
		ImGui::SetNextWindowPos({0.0F, 0.0F});
		ImGui::SetNextWindowSize(io.DisplaySize);
		auto window_flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar;
...
private:
		...
		for (int index = 1; index < argc; ++index) {
			...
			if (argument == "--mode") {
				...
				if (!piece_style_available(style)) {
					throw std::invalid_argument("--piece-style is not compiled into this executable");
				}
				...
				set_uci_option(simulator_.config(), "MultiPV", std::stoi(value()));
	...
	void load_settings() {
		...
		try {
			...
			stadium().set_match_limits(json.value("delay_ms", stadium().display_delay_ms()), json.value("max_plies", stadium().max_plies()));
			stadium().set_name(json.value("match_name", std::string{}));
			stadium().set_clock(json.value("clock_initial_ms", stadium().clock_initial_ms()), json.value("clock_increment_ms", stadium().clock_increment_ms()));
			...
			auto read_color = [&](const char *name, ImVec4 &target) {
				if (appearance.contains(name) && appearance[name].is_array() && appearance[name].size() == 4) {
					target = {appearance[name][0].get<float>(), appearance[name][1].get<float>(), appearance[name][2].get<float>(), appearance[name][3].get<float>()};
				}
			};
			...
			appearance_.coordinates = appearance.value("coordinates", appearance_.coordinates);
			appearance_.font_size = std::clamp(appearance.value("font_size", appearance_.font_size), 14.0F, 36.0F);
			appearance_.auto_font_scale = appearance.value("auto_font_scale", appearance_.auto_font_scale);
			const auto style = appearance.value("piece_style", appearance_.piece_style);
			if (piece_style_available(style)) {
				appearance_.piece_style = style;
			}
			theme_ = json.value("theme", std::string("dark")) == "light" ? Theme::Light : Theme::Dark;
		} catch (...) {
	...
	void save_settings() {
		const auto color_json = [](const ImVec4 &color) { return nlohmann::json::array({color.x, color.y, color.z, color.w}); };
		...
		const nlohmann::json json = {
		    ...
		    {"flipped", flipped_},
		    {"appearance",
		        {{"light", color_json(appearance_.light)}, {"dark", color_json(appearance_.dark)}, {"selected", color_json(appearance_.selected)},
		            {"last_move", color_json(appearance_.last_move)}, {"legal", color_json(appearance_.legal)}, {"coordinates", appearance_.coordinates},
		            {"font_size", appearance_.font_size}, {"auto_font_scale", appearance_.auto_font_scale}, {"piece_style", appearance_.piece_style}}},
		};
	...
	void render_menu_bar() {
		...
		const auto command_item = [](const char *label) {
			...
			return ImGui::Selectable(label, false, ImGuiSelectableFlags_None, size);
		};
		const auto popup_padding = ImGui::GetStyle().WindowPadding;
		ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, {8.0F, popup_padding.y});
		...
		if (ImGui::BeginMenu("Board")) {
			const bool position_locked = mode_ == Mode::Stadium && stadium().running();
	...
	void render_simulator(bool update_state) {
		...
		const auto layout = board_layout(available_width, available_height, simulator_board_fraction_);
		...
		if (const auto square = draw_board(visible, layout.board_size, flipped_, appearance_, viewing_history ? std::nullopt : selected_square_,
		        viewing_history ? std::vector<chess::Move>{} : legal_targets_, "simulator");
		    square && !viewing_history) {
			handle_simulator_square(*square);
		}
		if (const auto ply = render_history_slider("simulator-history", simulator_.game(), simulator_.viewed_ply())) {
		...
		render_board_splitter("simulator-board-splitter", layout, available_width, available_height, simulator_board_fraction_);
		ImGui::SameLine(0.0F, 0.0F);
		ImGui::BeginChild("simulator-right", {0.0F, 0.0F}, ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
		const auto panel_heights = right_panel_heights(ImGui::GetContentRegionAvail().y, simulator_panel_ratios_);
		ImGui::TextUnformatted("Suggested moves");
		analysis_table(simulator_.display(), simulator_.visible_game().board().getFen(), panel_heights[0]);
		render_panel_splitter("simulator-panel-splitter-1", simulator_panel_ratios_, 0, panel_heights.content_height);
		ImGui::TextUnformatted("Engine analysis");
		render_engine_summary(simulator_.display(), panel_heights[1]);
		render_panel_splitter("simulator-panel-splitter-2", simulator_panel_ratios_, 1, panel_heights.content_height);
	...
	void handle_simulator_square(int square) {
		...
		if (piece != chess::Piece::NONE && piece.color() == simulator_.game().board().sideToMove()) {
	...
	void render_engine_summary(const AnalysisSnapshot &snapshot, float height) {
		if (ImGui::BeginChild("engine-summary", {0.0F, height}, ImGuiChildFlags_Borders)) {
			ImGui::Text("Engine: %s", snapshot.engine_name.empty() ? "-" : snapshot.engine_name.c_str());
			ImGui::Text("State: %s", snapshot.searching ? "thinking" : snapshot.finished ? "complete" : "idle");
			if (!snapshot.lines.empty()) {
				...
				ImGui::Text("Nodes: %llu   NPS: %llu   Time: %d ms", static_cast<unsigned long long>(line.nodes), static_cast<unsigned long long>(line.nps), line.elapsed_ms);
			}
		}
		ImGui::EndChild();
	}
	...
	void render_board_state(const GameState &game, float height) {
		if (ImGui::BeginChild("board-state", {0.0F, height}, ImGuiChildFlags_Borders)) {
	...
	void render_stadium() {
		...
		const bool human_input = !stadium().viewed_ply() && stadium().human_to_move();
		const float available_height = ImGui::GetContentRegionAvail().y;
		const float available_width = ImGui::GetContentRegionAvail().x;
		const auto layout = board_layout(available_width, available_height, stadium_board_fraction_);
		ImGui::PushStyleColor(ImGuiCol_ChildBg, ImGui::GetStyleColorVec4(ImGuiCol_WindowBg));
		ImGui::BeginChild("stadium-left", {layout.board_size, 0.0F});
		ImGui::PopStyleColor();
		if (const auto square = draw_board(visible, layout.board_size, flipped_, appearance_, human_input ? selected_square_ : std::nullopt,
		        human_input ? legal_targets_ : std::vector<chess::Move>{}, "stadium");
		    square && human_input) {
			handle_stadium_square(*square);
		}
		if (const auto ply = render_history_slider("stadium-history", stadium().game(), stadium().viewed_ply())) {
		...
		ImGui::EndChild();
		ImGui::SameLine(0.0F, 0.0F);
		render_board_splitter("stadium-board-splitter", layout, available_width, available_height, stadium_board_fraction_);
		ImGui::SameLine(0.0F, 0.0F);
		ImGui::BeginChild("stadium-right", {0.0F, 0.0F}, ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
		const auto panel_heights = right_panel_heights(ImGui::GetContentRegionAvail().y, stadium_panel_ratios_);
		const bool white_turn = stadium().game().board().sideToMove() == chess::Color::WHITE;
		ImGui::Text("%s to move", white_turn ? stadium().white_name().c_str() : stadium().black_name().c_str());
		analysis_table(stadium().display(), stadium().game().board().getFen(), panel_heights[0]);
		render_panel_splitter("stadium-panel-splitter-1", stadium_panel_ratios_, 0, panel_heights.content_height);
		ImGui::TextUnformatted("Match");
		if (ImGui::BeginChild("match-state", {0.0F, panel_heights[1]}, ImGuiChildFlags_Borders)) {
			ImGui::TextUnformatted(stadium().display_name().c_str());
			ImGui::Text("%s  vs  %s", stadium().white_name().c_str(), stadium().black_name().c_str());
			if (stadium().clock_enabled()) {
				...
				ImGui::Text("Clock: %s  /  %s", white_clock.c_str(), black_clock.c_str());
			}
			ImGui::Text("State: %s", stadium().status().c_str());
			ImGui::Text("Ply: %zu / %d", stadium().game().plies(), stadium().max_plies());
			ImGui::Text("Result: %s", stadium().result().c_str());
			if (!stadium().termination().empty()) {
				ImGui::Text("Termination: %s", stadium().termination().c_str());
			}
		}
		ImGui::EndChild();
		render_panel_splitter("stadium-panel-splitter-2", stadium_panel_ratios_, 1, panel_heights.content_height);
	...
	void handle_stadium_square(int square) {
		...
		if (piece != chess::Piece::NONE && piece.color() == stadium().game().board().sideToMove()) {
			selected_square_ = square;
	...
	void start_stadium() {
		try {
			validate_match_side("White", stadium().white_config(), stadium().white_config().path.empty());
			validate_match_side("Black", stadium().black_config(), stadium().black_config().path.empty());
			stadium().start();
			selected_square_.reset();
			legal_targets_.clear();
		} catch (const std::exception &error) {
	...
	StadiumSession &stadium() { return stadiums_.active(); }

	const StadiumSession &stadium() const { return stadiums_.active(); }

	/// Resizes the square board and right panel within a fixed application window.
	void render_board_splitter(const char *identifier, const BoardLayout &layout, float available_width, float available_height, float &board_fraction) {
		ImGui::PushID(identifier);
		ImGui::InvisibleButton("##splitter", {layout.splitter_width, available_height});
		const bool hovered = ImGui::IsItemHovered();
		...
		if (active) {
			const float requested = std::clamp(layout.board_size + ImGui::GetIO().MouseDelta.x, layout.minimum_board_size, layout.maximum_board_size);
			board_fraction = requested / std::max(1.0F, available_width);
		}
		const ImVec2 minimum = ImGui::GetItemRectMin();
		const ImVec2 maximum = ImGui::GetItemRectMax();
		const float x = (minimum.x + maximum.x) * 0.5F;
		ImGui::GetWindowDrawList()->AddLine({x, minimum.y + 4.0F}, {x, maximum.y - 4.0F},
	...
	RightPanelHeights right_panel_heights(float available_height, const std::array<float, 3> &ratios) const {
		constexpr float splitter_height = 8.0F;
		const float headers = ImGui::GetTextLineHeightWithSpacing() * 3.0F;
		const float content_height = std::max(3.0F, available_height - headers - splitter_height * 2.0F);
	...
	void render_panel_splitter(const char *identifier, std::array<float, 3> &ratios, std::size_t first, float content_height) {
		constexpr float splitter_height = 8.0F;
		ImGui::PushID(identifier);
		...
		if (active && first + 1 < ratios.size()) {
			...
			const float minimum = std::min(pair * 0.45F, 80.0F / std::max(1.0F, content_height));
			const float resized = std::clamp(ratios[first] + ImGui::GetIO().MouseDelta.y / std::max(1.0F, content_height), minimum, pair - minimum);
		...
		ImGui::GetWindowDrawList()->AddLine({minimum.x + 4.0F, y}, {maximum.x - 4.0F, y},
		    ImGui::GetColorU32(active ? ImGuiCol_SeparatorActive
		            : hovered         ? ImGuiCol_SeparatorHovered
		                              : ImGuiCol_Separator),
		    active ? 2.0F : 1.0F);
	...
	std::optional<std::size_t> render_history_slider(const char *identifier, const GameState &live, std::optional<std::size_t> view_ply) {
		ImGui::Spacing();
		const int maximum = static_cast<int>(live.plies());
		int position = view_ply ? std::min(static_cast<int>(*view_ply), maximum) : maximum;
		ImGui::PushID(identifier);
		const float width = std::max(1.0F, ImGui::GetContentRegionAvail().x);
		const float height = ImGui::GetFrameHeight();
		ImGui::InvisibleButton("##position", {width, height}, ImGuiButtonFlags_MouseButtonLeft);
		const ImVec2 minimum = ImGui::GetItemRectMin();
		...
		const float pointer_ratio = std::clamp((ImGui::GetIO().MousePos.x - track_left) / std::max(1.0F, track_right - track_left), 0.0F, 1.0F);
		bool changed = false;
		if (maximum > 0 && ImGui::IsItemActive()) {
			const int selected = std::clamp(static_cast<int>(std::lround(pointer_ratio * maximum)), 0, maximum);
		...
		const float position_ratio = maximum > 0 ? static_cast<float>(position) / static_cast<float>(maximum) : 1.0F;
		const float visual_ratio = maximum > 0 && ImGui::IsItemActive() ? pointer_ratio : position_ratio;
		const float knob_x = track_left + (track_right - track_left) * visual_ratio;
		auto *draw = ImGui::GetWindowDrawList();
		const ImU32 track_color = ImGui::GetColorU32(ImGuiCol_FrameBg);
		const ImU32 fill_color = ImGui::GetColorU32(maximum == 0 ? ImGuiCol_TextDisabled : ImGuiCol_SliderGrab);
		const ImU32 knob_color = ImGui::GetColorU32(ImGui::IsItemActive() ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
		const float track_half = std::max(2.0F, height * 0.09F);
		draw->AddRectFilled({track_left, center_y - track_half}, {track_right, center_y + track_half}, track_color, track_half);
		draw->AddRectFilled({track_left, center_y - track_half}, {knob_x, center_y + track_half}, fill_color, track_half);
		draw->AddCircleFilled({knob_x, center_y}, knob_radius, knob_color, 24);
		draw->AddCircle({knob_x, center_y}, knob_radius, ImGui::GetColorU32(ImGuiCol_Border), 24, std::max(1.0F, style.FrameBorderSize));
		ImGui::PopID();
		if (!changed) {
			return std::nullopt;
		}
	...
	void open_settings(Mode mode) {
		...
		if (mode == Mode::Simulator) {
			...
			clock_minutes_edit_ = static_cast<int>(stadium().clock_initial_ms() / 60'000);
			clock_increment_seconds_edit_ = static_cast<int>(stadium().clock_increment_ms() / 1000);
	...
	void import_uci_engine() {
		if (const auto path = file_dialog(false, L"Executables\0*.exe\0All files\0*.*\0", L"exe")) {
			try {
				EngineConfig engine;
				engine.path = *path;
				engine.name = path->stem().string();
				engine.discovered_options = UciEngine::discover_options(engine);
	...
	void import_simulator_pgn() {
		if (const auto path = file_dialog(false, L"PGN files\0*.pgn\0All files\0*.*\0", L"pgn")) {
			try {
	...
	void toggle_stadium_pause() { stadium().toggle_pause(); }

	/// Opens a focused FEN editor for the active mode.
	void open_position_editor() {
		position_mode_ = mode_;
		position_edit_ = mode_ == Mode::Simulator ? simulator_.start_fen() : stadium().start_fen();
	...
	void render_position_editor() {
		...
		if (!ImGui::BeginPopupModal("FEN", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings)) {
			return;
		}
		ImGui::TextUnformatted(position_mode_ == Mode::Simulator ? "Simulator start position" : "Stadium start position");
		ImGui::SetNextItemWidth(-1.0F);
		const bool submitted = ImGui::InputTextWithHint("##position-fen", "FEN or startpos", &position_edit_, ImGuiInputTextFlags_EnterReturnsTrue);
	...
	void option_editor(EngineConfig &config) {
		if (config.discovered_options.empty()) {
			ImGui::TextDisabled("Engine options will appear after a successful UCI handshake.");
			return;
		}
		...
		for (const auto &option : config.discovered_options) {
			...
			if (option.type == "check") {
				...
				bool value = initial == "true" || initial == "1" || initial == "on";
				...
				if (ImGui::InputScalar("##value", ImGuiDataType_S64, &value, &step)) {
					if (option.minimum) {
				...
				if (ImGui::Button(option.name.c_str())) {
					if (std::find(config.button_commands.begin(), config.button_commands.end(), option.name) == config.button_commands.end()) {
				...
				ImGui::TextDisabled("%s (%s)", option.name.c_str(), option.type.c_str());
			}
			ImGui::PopID();
		}
		if (changed) {
	...
	void engine_editor(const char *identifier, EngineConfig &config, bool include_name = true) {
		ImGui::PushID(identifier);
		ImGui::SeparatorText("Engine");
		if (!registry_.engines().empty()) {
			const std::string preview = config.name.empty() ? config.path.filename().string() : config.name;
			if (ImGui::BeginCombo("Imported engine", preview.empty() ? "Select engine" : preview.c_str())) {
				for (const auto &engine : registry_.engines()) {
					const bool selected = engine.path == config.path;
					const std::string label = engine.name.empty() ? engine.path.filename().string() : engine.name;
		...
		const float path_width = std::max(80.0F, ImGui::GetContentRegionAvail().x - browse_width - ImGui::GetStyle().ItemSpacing.x);
		...
		if (ImGui::Button("Browse")) {
			if (const auto selected = file_dialog(false, L"Executables\0*.exe\0All files\0*.*\0", L"exe")) {
				try {
		...
		if (ImGui::CollapsingHeader("Launch arguments")) {
			ImGui::TextDisabled("Optional process arguments passed before the UCI handshake.");
			ImGui::SetNextItemWidth(-1.0F);
	...
	void match_side_editor(const char *identifier, EngineConfig &config, bool &human) {
		ImGui::PushID(identifier);
		...
		ImGui::EndDisabled();
		if (human) {
			ImGui::TextDisabled("Moves are entered directly on the board when this side is to move.");
		}
		ImGui::PopID();
	}

	/// Requires stable PGN identity for every configured UCI match participant.
	void validate_match_side(const char *side, const EngineConfig &config, bool human) const {
		if (config.name.empty()) {
			throw std::invalid_argument(std::string(side) + " participant name is required for PGN");
		}
		if (!human && config.path.empty()) {
			throw std::invalid_argument(std::string(side) + " engine executable is required");
		}
	}
	...
	void render_matches() {
		...
		if (!ImGui::BeginPopupModal("Matches", nullptr, ImGuiWindowFlags_NoSavedSettings)) {
			return;
		}
		...
		if (match_create_mode_) {
			...
			const std::string default_name = "#" + std::to_string(next_id) + " " + side_name(new_white_, new_white_human_) + " vs. " + side_name(new_black_, new_black_human_);
			ImGui::TextUnformatted("New match");
			ImGui::SetNextItemWidth(-1.0F);
			ImGui::InputTextWithHint("##new-match-name", default_name.c_str(), &new_match_name_);
			ImGui::BeginChild("new-match-content", {0.0F, -48.0F}, ImGuiChildFlags_None, ImGuiWindowFlags_AlwaysVerticalScrollbar);
			if (ImGui::BeginTabBar("new-match-tabs")) {
				if (ImGui::BeginTabItem("White")) {
					match_side_editor("new-white", new_white_, new_white_human_);
					ImGui::EndTabItem();
				}
				if (ImGui::BeginTabItem("Black")) {
					match_side_editor("new-black", new_black_, new_black_human_);
					ImGui::EndTabItem();
				}
				if (ImGui::BeginTabItem("Match")) {
					...
					new_increment_seconds_ = std::max(0, new_increment_seconds_);
			...
			if (ImGui::Button("Create", {100.0F, 0.0F})) {
				try {
					validate_match_side("White", new_white_, new_white_human_);
					validate_match_side("Black", new_black_, new_black_human_);
					if (!new_white_human_) {
					...
					session.set_clock(static_cast<std::int64_t>(new_clock_minutes_) * 60'000, static_cast<std::int64_t>(new_increment_seconds_) * 1000);
		...
		if (ImGui::BeginTable("match-list", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable | ImGuiTableFlags_SizingStretchProp, {0.0F, -54.0F})) {
			...
			for (std::size_t index = 0; index < stadiums_.size(); ++index) {
				...
				ImGui::TableSetColumnIndex(4);
				if (ImGui::SmallButton(index == stadiums_.active_index() ? "Current" : "Enter")) {
	...
	void appearance_editor(Appearance &appearance, Theme &theme) {
		...
		ImGui::ColorEdit4("Light squares", &appearance.light.x, ImGuiColorEditFlags_NoInputs);
		ImGui::ColorEdit4("Dark squares", &appearance.dark.x, ImGuiColorEditFlags_NoInputs);
		ImGui::ColorEdit4("Selection", &appearance.selected.x, ImGuiColorEditFlags_NoInputs);
		ImGui::ColorEdit4("Last move", &appearance.last_move.x, ImGuiColorEditFlags_NoInputs);
	...
	void render_settings() {
		...
		if (!ImGui::BeginPopupModal("Settings", nullptr, ImGuiWindowFlags_NoSavedSettings)) {
			return;
		}
		ImGui::BeginChild("settings-content", {0.0F, -48.0F}, ImGuiChildFlags_None, ImGuiWindowFlags_AlwaysVerticalScrollbar);
		if (settings_mode_ == Mode::Simulator) {
			...
			if (ImGui::BeginTabItem("White")) {
				match_side_editor("white-edit", white_edit_, white_human_edit_);
				ImGui::EndTabItem();
			}
			if (ImGui::BeginTabItem("Black")) {
				match_side_editor("black-edit", black_edit_, black_human_edit_);
				ImGui::EndTabItem();
			}
			if (ImGui::BeginTabItem("Match")) {
				ImGui::InputTextWithHint("Name", stadium().display_name().c_str(), &match_name_edit_);
				...
				clock_increment_seconds_edit_ = std::max(0, clock_increment_seconds_edit_);
		...
		if (ImGui::Button("Apply", {100.0F, 0.0F})) {
			try {
				const auto primary_options = settings_mode_ == Mode::Simulator || !white_human_edit_
				    ? option_values(settings_mode_ == Mode::Simulator ? simulator_edit_.options : white_edit_.options)
				    : nlohmann::json::object();
				if (settings_mode_ == Mode::Stadium) {
					validate_match_side("White", white_edit_, white_human_edit_);
					validate_match_side("Black", black_edit_, black_human_edit_);
					const auto secondary_options = black_human_edit_ ? nlohmann::json::object() : option_values(black_edit_.options);
				...
				if (settings_mode_ == Mode::Simulator) {
					...
					stadium().set_clock(static_cast<std::int64_t>(clock_minutes_edit_) * 60'000, static_cast<std::int64_t>(clock_increment_seconds_edit_) * 1000);
	...
	void save_pgn(
	    const GameState &game, const std::string &white, const std::string &black, const std::string &result_override = "", const std::string &termination_override = "") {
		if (const auto path = file_dialog(true, L"PGN files\0*.pgn\0All files\0*.*\0", L"pgn")) {
			try {
				std::ofstream output(*path, std::ios::trunc);
				if (!output) {
					throw std::runtime_error("could not open PGN output");
				}
				output << game.pgn(white, black, result_override, termination_override);
			} catch (const std::exception &error) {
				show_error(error.what());
			}
		}
...
void refresh_window(GLFWwindow *window, bool update_state) {
	auto *context = static_cast<RefreshContext *>(glfwGetWindowUserPointer(window));
	...
	if (context->last_render != Clock::time_point{} && now - context->last_render < frame_interval) {
		return;
	}
...
void defer_window_drawing(GLFWwindow *window) {
	auto *context = static_cast<RefreshContext *>(glfwGetWindowUserPointer(window));
	if (context != nullptr) {
		context->defer_drawing_until = Clock::now() + std::chrono::milliseconds(100);
	}
}
...
int run_application(int argc, char **argv) {
	...
	if (!glfwInit()) {
		throw std::runtime_error(latest_glfw_error.empty() ? "could not initialize GLFW" : "could not initialize GLFW: " + latest_glfw_error);
	}
...
#endif
	GLFWwindow *window = glfwCreateWindow(1280, 820, "Gadidae", nullptr, nullptr);
	if (window == nullptr) {
		glfwTerminate();
		throw std::runtime_error(latest_glfw_error.empty() ? "could not create the OpenGL window" : "could not create the OpenGL window: " + latest_glfw_error);
	}
