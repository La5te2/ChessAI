...
int main() {
	try {
		...
		require(game.position_at(4).uci_position() == first_cycle,
				"historical UCI position included the wrong move range");
		require(game.uci_position() == first_cycle + " g1f3 g8f6 f3g1 f6g8",
				"live UCI position omitted move history");
