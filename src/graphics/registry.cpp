...
std::size_t Registry::add(EngineConfig config) {
	...
	const auto existing = std::find_if(
		engines_.begin(), engines_.end(), [&](const EngineConfig &engine) {
			return normalized_path(engine.path) == config.path;
		});
	if(existing != engines_.end()) {
		*existing = std::move(config);
		return static_cast<std::size_t>(
			std::distance(engines_.begin(), existing));
	}
...
std::filesystem::path Registry::normalized_path(
