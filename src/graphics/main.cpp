...
std::string utf8_argument(const wchar_t *value) {
	...
	WideCharToMultiByte(CP_UTF8, 0, value, -1, output.data(), length, nullptr,
						nullptr);
	output.pop_back();
	return output;
}
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
	...
	if(wide_arguments == nullptr) {
		MessageBoxA(nullptr, "Could not parse the Windows command line.", "Gadidae",
					MB_OK | MB_ICONERROR);
		return 1;
	}
	try {
		...
		const std::string message =
			std::string("Gadidae graphics error:\n") + error.what();
		MessageBoxA(nullptr, message.c_str(), "Gadidae", MB_OK | MB_ICONERROR);
		return 1;
	}
}
