...
std::span<const std::uint8_t> embedded_archive() {
	...
	const HRSRC resource = FindResourceW(module, MAKEINTRESOURCEW(piece_archive_resource), MAKEINTRESOURCEW(10));
	if (resource == nullptr) {
		return {};
	}
	...
	return data == nullptr ? std::span<const std::uint8_t>() : std::span<const std::uint8_t>(static_cast<const std::uint8_t *>(data), size);
#else
	return {gadidae_piece_archive_start, static_cast<std::size_t>(gadidae_piece_archive_end - gadidae_piece_archive_start)};
#endif
}
...
std::vector<std::uint8_t> decompress_archive(std::span<const std::uint8_t> compressed) {
	if (compressed.size() < 12 || std::memcmp(compressed.data(), "GPCZ", 4) != 0) {
		throw std::runtime_error("invalid compressed piece archive");
	}
	std::uint64_t raw_size = 0;
	std::memcpy(&raw_size, compressed.data() + 4, sizeof(raw_size));
	if (raw_size > maximum_archive_size || raw_size > std::numeric_limits<uLongf>::max()) {
	...
	const int status = uncompress(raw.data(), &destination_size, compressed.data() + 12, static_cast<uLong>(compressed.size() - 12));
...
std::vector<ArchivedPieceStyle> decode_archive() {
	...
	for (std::uint32_t style_index = 0; style_index < style_count; ++style_index) {
		...
		for (auto &mesh : style.pieces) {
			...
			if (vertex_count > std::numeric_limits<std::uint16_t>::max() || index_count % 3 != 0) {
