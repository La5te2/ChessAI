...
nlohmann::json evaluate_models(const ArenaOptions &options) {
	...
	if (options.games <= 0 || options.games_in_flight <= 0 || options.max_plies <= 0) {
		throw std::invalid_argument("arena game counts and max plies must be positive");
	}
	...
	CheckpointInfo candidate_info;
	CheckpointInfo baseline_info;
	auto candidate_model = load_checkpoint(options.candidate, device, &candidate_info);
	auto baseline_model = load_checkpoint(options.baseline, device, &baseline_info);
