...
# Launch a production Melano FCPI run for a 16 GiB RTX 4080 Super.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${ROOT_DIR}"

FCPI="${FCPI:-build/melano/fcpi}"
MODEL="${MODEL:-models/melano/melano.pth}"
DEVICE="${DEVICE:-cuda}"
PRECISION="${PRECISION:-bf16}"

ITERATIONS="${ITERATIONS:-5}"
GAMES_PER_ITER="${GAMES_PER_ITER:-1000}"
GAMES_IN_FLIGHT="${GAMES_IN_FLIGHT:-64}"
MAX_PLIES="${MAX_PLIES:-240}"

OPENING_BOOK="${OPENING_BOOK:-data/openings.gen.bin}"
STARTPOS_FRACTION="${STARTPOS_FRACTION:-0.5}"
BOOK_PLIES="${BOOK_PLIES:-8}"
MAX_BOOK_POSITIONS="${MAX_BOOK_POSITIONS:-50000}"

INFERENCE_BATCH_SIZE="${INFERENCE_BATCH_SIZE:-64}"
TARGET_RECORDS_PER_BATCH="${TARGET_RECORDS_PER_BATCH:-256}"
COUNTERFACTUAL_BUDGET="${COUNTERFACTUAL_BUDGET:-24}"

BEHAVIOR_TEMPERATURE="${BEHAVIOR_TEMPERATURE:-0.85}"

POLICY_WEIGHT="${POLICY_WEIGHT:-1.0}"
VALUE_WEIGHT="${VALUE_WEIGHT:-1.0}"
DUELING_Q_WEIGHT="${DUELING_Q_WEIGHT:-0.5}"
DYNAMICS_WEIGHT="${DYNAMICS_WEIGHT:-0.25}"
IMAGINED_VALUE_WEIGHT="${IMAGINED_VALUE_WEIGHT:-0.25}"
TARGET_DECAY="${TARGET_DECAY:-0.995}"
...
EVAL_GAMES_IN_FLIGHT="${EVAL_GAMES_IN_FLIGHT:-32}"
...
EVAL_MCTS_BATCH_SIZE="${EVAL_MCTS_BATCH_SIZE:-64}"
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
	printf '%s\n' \
		"usage: bash scripts/melano_fcpi.sh" \
		"" \
		"Environment overrides:" \
		"  MODEL=models/melano/melano.pth ITERATIONS=5 GAMES_PER_ITER=1000" \
		"  PRECISION=bf16 BATCH_SIZE=256 INFERENCE_BATCH_SIZE=64 EVAL_GAMES=400" \
		"" \
...
if [[ ! -x "${FCPI}" ]]; then
	echo "Melano FCPI executable is missing: ${FCPI}" >&2
	echo "Run: bash scripts/build.sh" >&2
	exit 1
fi
if [[ ! -f "${MODEL}" ]]; then
	echo "Melano model is missing: ${MODEL}" >&2
	exit 1
fi
...
COMMAND=(
	...
	--imagined-value-weight "${IMAGINED_VALUE_WEIGHT}"
	--target-decay "${TARGET_DECAY}"
	--epochs "${EPOCHS}"
...
mkdir -p data/runs
LAUNCH_LOG="data/runs/.melano_fcpi_$(date +%Y%m%d_%H%M%S)_$$.log"

echo "Melano FCPI launch"
...
echo "training: batch_size=${BATCH_SIZE} epochs=${EPOCHS} max_steps=${TRAIN_MAX_STEPS} lr=${LEARNING_RATE} target_decay=${TARGET_DECAY} grad_clip=${GRAD_CLIP}"
...
for _ in {1..100}; do
	...
	if ! kill -0 "${PID}" 2>/dev/null; then
		echo "Melano FCPI exited before creating a run." >&2
		cat "${LAUNCH_LOG}" >&2
		exit 1
	fi
	sleep 0.1
...
if [[ -z "${RUN_ID}" ]]; then
	echo "Melano FCPI started, but its run id was not observed within 10 seconds." >&2
	echo "pid=${PID}" >&2
