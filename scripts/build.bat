...
set "BUILD_ELEGINUS_TRAINING=ON"
...
if /I "%GADIDAE_BUILD_ELEGINUS_TRAINING%"=="0" set "BUILD_ELEGINUS_TRAINING=OFF"
if /I "%GADIDAE_BUILD_ELEGINUS_TRAINING%"=="OFF" set "BUILD_ELEGINUS_TRAINING=OFF"
if /I "%GADIDAE_BUILD_ELEGINUS_TRAINING%"=="FALSE" set "BUILD_ELEGINUS_TRAINING=OFF"
if "%BUILD_ELEGINUS%"=="OFF" set "BUILD_ELEGINUS_TRAINING=OFF"
...
if "%BUILD_ELEGINUS%"=="ON" if "%BUILD_ELEGINUS_TRAINING%"=="ON" set "VERIFY_TORCH=ON"
if "%BUILD_GADUS%"=="ON" set "VERIFY_HDF5=ON"
if "%BUILD_MELANO%"=="ON" set "VERIFY_HDF5=ON"
if "%BUILD_ELEGINUS%"=="ON" if "%BUILD_ELEGINUS_TRAINING%"=="ON" set "VERIFY_HDF5=ON"
if "%BUILD_GADUS%"=="ON" set "VERIFY_ZLIB=ON"
if "%BUILD_MELANO%"=="ON" set "VERIFY_ZLIB=ON"
if "%BUILD_ELEGINUS%"=="ON" if "%BUILD_ELEGINUS_TRAINING%"=="ON" set "VERIFY_ZLIB=ON"
...
cmake -S "%ROOT_DIR%" -B "%WORK_DIR%" -G Ninja -DCMAKE_BUILD_TYPE=Release -DGADIDAE_BUILD_GADUS="%BUILD_GADUS%" -DGADIDAE_BUILD_MELANO="%BUILD_MELANO%" -DGADIDAE_BUILD_ELEGINUS="%BUILD_ELEGINUS%" -DGADIDAE_BUILD_ELEGINUS_TRAINING="%BUILD_ELEGINUS_TRAINING%" -DGADIDAE_BUILD_GRAPHICS="%BUILD_GRAPHICS%" -DGADIDAE_TORCH_DIR="%TORCH_DIR%" || goto :failed
...
if "%BUILD_ELEGINUS%"=="ON" (
	...
	if "%BUILD_ELEGINUS_TRAINING%"=="ON" (
		for %%F in (preprocess train embed tests) do (
			if not exist "%WORK_DIR%\eleginus\%%F.exe" goto :failed
			copy /y "%WORK_DIR%\eleginus\%%F.exe" "%PUBLISH_DIR%\eleginus\%%F.exe" >nul || goto :failed
		)
...
if "%BUILD_ELEGINUS%"=="ON" if "%BUILD_ELEGINUS_TRAINING%"=="OFF" echo Eleginus LibTorch training tools skipped.
if "%BUILD_GRAPHICS%"=="ON" (
