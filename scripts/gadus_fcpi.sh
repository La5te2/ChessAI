...
GAMES_PER_ITER="${GAMES_PER_ITER:-6000}"
GAMES_IN_FLIGHT="${GAMES_IN_FLIGHT:-512}"
MAX_PLIES="${MAX_PLIES:-240}"

OPENING_BOOK="${OPENING_BOOK:-data/openings.sam.bin}"
MAX_BOOK_POSITIONS="${MAX_BOOK_POSITIONS:-10000}"
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
	printf '%s\n' \
		...
		"  MODEL=models/gadus/gadus.pth ITERATIONS=5 GAMES_PER_ITER=6000" \
