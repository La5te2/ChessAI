...
/// Parses the exact public precision names used by Gadus command-line programs.
...
/// Returns the stable external spelling of one Gadus compute precision.
inline const char *compute_precision_name(ComputePrecision value) noexcept {
	return value == ComputePrecision::Bf16 ? "bf16" : "fp32";
}

/// Rejects reduced precision on devices where this implementation has no calibrated path.
